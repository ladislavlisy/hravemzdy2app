...
- (id)initWithTagCode:(NSUInteger)tagCode andValues:(NSDictionary *)values {
    if (!(self=[super initWithCodeName:[PYGSymbolConcepts codeRef:CONCEPT_TAX_ADVANCE_BASE] andTagCode:tagCode])) return nil;
    return self;
}

+ (PYGTaxAdvanceBaseConcept *)conceptWithTagCode:(NSUInteger)tagCode andValues:(NSDictionary *)values {
...
- (PYGTaxAdvanceBaseConcept*)newConceptWithCode:(NSUInteger)tagCode andValues:(NSDictionary *)values {
    PYGTaxAdvanceBaseConcept *concept = [PYGTaxAdvanceBaseConcept conceptWithTagCode:tagCode andValues:values];
...
-(NSDecimalNumber *)taxRoundedBase:(PYGPayrollPeriod *)period withDecl:(BOOL)taxDecl income:(NSDecimalNumber *)taxIncome base:(NSDecimalNumber *)taxBase {
    NSUInteger year = period.year;

    NSNumber * taxWithholdMaximum = @([self taxWithholdMax:year]);

    if (taxDecl == YES)
    {
        return [self advanceRoundedBase:period withDecl:taxDecl base:taxBase];
    }
    else if ([self isGreater:taxIncome thenDecimal:DECIMAL_NUMB(taxWithholdMaximum)])
    {
        return [self advanceRoundedBase:period withDecl:taxDecl base:taxBase];
    }
    else
...
-(NSDecimalNumber *)advanceRoundedBase:(PYGPayrollPeriod *)period withDecl:(BOOL)taxDecl base:(NSDecimalNumber *)taxBase {
    ...
    NSDecimalNumber * amountTaxable = [self maxToBig:NSDecimalNumber.zero andDecimal:taxBase];

    if ([self isGreater:amountTaxable thenDecimal:amountHundred] == YES)
    {
        return [self bigDecimal:amountTaxable nearRoundUp:@100];
    }
    else if (year >= 2011)
    {
        return [self bigTaxRoundUp:amountTaxable];
    }
    else
    {
        return [self bigTaxRoundDown:amountTaxable];
    }
}

-(NSUInteger)taxWithholdMax:(NSUInteger) year {
    if (year>=2004) {
        return 5000;
    }
    else if (year>=2001) {
